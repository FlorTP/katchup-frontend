...
export class FamiliaService {
  ...
  misFamilias() {
    return this.http.get<any[]>(`${environment.apiUrl}/familias/mis-familias`).pipe(
      tap((list) => {
  ...
  agregarMiembro(familiaId: string, dto: any) {
    return this.http.post<MiembroFamilia>(
      `${environment.apiUrl}/familias/${familiaId}/miembros`, dto
    );
  }

  actualizarMiembro(familiaId: string, miembroId: string, dto: any) {
    return this.http.put(
      `${environment.apiUrl}/familias/${familiaId}/miembros/${miembroId}`, dto
    );
  }
  ...
  unirseConCodigo(codigo: string) {
    return this.http.post<{ familia: Familia; miembro: MiembroFamilia }>(
